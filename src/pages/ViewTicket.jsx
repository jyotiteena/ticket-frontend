...
const ViewTicket = () => {
    ...
    async function show() {
        const res = await axios.get("https://ticket-backend-j3d7.onrender.com/api/ticket")
    ...
    const handleActionChange = async (ticketId, actions) => {
        try {
            const res=await axios.put(`https://ticket-backend-j3d7.onrender.com/api/ticket/${ticketId}`, { actions },);
            if(res.data.success){
